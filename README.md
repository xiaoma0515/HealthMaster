...
| 提醒提示音 | **默认开启**，可从托盘一键关闭。到点响一声禅意颂钵（约 2.9s；四类同时到点也**只响一次**）；单击图标完成时是一声很轻的「檐下滴水」确认音。**纯代码合成，仓库里没有任何音频文件**；勿扰期间与清空图标时绝不出声 |
| 托盘菜单 | 暂停/恢复全部、关闭/开启提醒声音、打开配置文件夹、退出 |

**不做**：开机自启、完整设置界面（预留接缝，见下）。
...
- `SoundEnabled`：提醒提示音总开关，**默认 `true`**（老配置文件里没有这个键时同样按开启处理）。
  用托盘菜单「关闭提醒声音 / 开启提醒声音」切换即时生效并写回此项，**无需重启**。
  关闭状态下启动的进程完全不做音频合成（省掉数秒 CPU）。
...
└─ src/HealthMaster/
   ...
   ├─ Services/                      # 调度、配置读写、勿扰判定、电源监听、托盘、提示音
   ├─ ViewModels/                    # FloatingViewModel、ReminderBadgesViewModel/Item
   ├─ Views/                         # FloatingWindow、ReminderBadgeWindow（提醒图标）
   ├─ Themes/                        # 深色 HUD 视觉资源 + 托盘菜单主题
   └─ Resources/                     # Strings（中文文案）、IconGeometries（矢量图标）、ZenTones（提示音合成）
```

> **零二进制资源**：图标是矢量路径、托盘图标运行时绘制、提示音在内存里合成——
> 仓库与产物里都没有任何图片 / 字体 / 音频文件。
...
4. **无开机自启**。提醒仍是**非打断式**：只冒图标 + 一声提示音，不弹窗、不闪任务栏、不抢焦点，用户不点则一直挂着。
   提示音**音量不可调**（只有开 / 关），如嫌吵请从托盘关掉。
   声音在启动后约 7 秒于低优先级后台线程合成完毕（一次性），此后每次播放开销可忽略。
